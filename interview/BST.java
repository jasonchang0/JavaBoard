...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    public Key floor_key(Key key) {
        return floor_key(root, key, null);
    }

    private Key floor_key(Node x, Key key, Key best) {
        if (x == null) return best;
        int cmp = key.compareTo(x.key);
        if (cmp < 0) return floor_key(x.left, key, best);
        else if (cmp > 0) return floor_key(x.right, key, x.key);
        else return x.key;
    }
